...
int maxCliqueHeu(CGraphIO* gio, vector<int>* max_clique_data) {
  ...
  for (size_t iCandidateVertex = 0; iCandidateVertex < p_v_i_Vertices->size() - 1;
       iCandidateVertex++) {
    // Pruning 1
...
int maxCliqueHeuIncremental(CGraphIO* gio,
  ...
  int maxClq = prev_maxclique_size, icc;
  vector<int> v_i_S;
  vector<int> v_i_S1;
  v_i_S.resize(maxDegree + 1, 0);
  v_i_S1.resize(maxDegree + 1, 0);
