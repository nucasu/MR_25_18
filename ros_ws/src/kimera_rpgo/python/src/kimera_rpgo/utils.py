...
class DebugType(Enum):  # extra visualization type TODO(Yun) re-evaluate
    UNKNOWN = 0
    TRUE_POSITIVE = 1
    TRUE_NEGATIVE = 2
    FALSE_POSITIVE = 3
...
class Pose3D:
    ...
    def random(
        cls,
        t_min=np.array([-100, -100, -100]),
        t_max=np.array([100, 100, 100]),
        identity_rot=False,
    ):
    ...
    def from_matrix(cls, matrix):
        t = matrix[0:3, 3]
        R = Rot.from_matrix(matrix[0:3, 0:3])
        return cls(t, R)
...
class Edge:
    def __init__(
        self, key_from, key_to, edge_type, pose, covar, debug=DebugType.UNKNOWN
    ):
    ...
    def copy(self):
        return Edge(
            self.key_from, self.key_to, self.type, self.pose, self.covar, self.debug
        )
...
def node_from_g2o_vertex(g2o_line, is_3d):
    if not is_3d:
        if g2o_line.startswith("VERTEX_SE2"):
            ...
            raise Exception("Unhandled input node type {}".format(g2o_line.split()[0]))
...
def edge_from_g2o_edge(g2o_line, is_3d):
    if not is_3d:
        ...
        if g2o_line.split()[0] == "EDGE_SE2":
            ...
            raise Exception("Unhandled input edge type {}".format(g2o_line.split()[0]))
        pose = Pose2D(g2o_line)
