...
/* ************************************************************************* */
TEST(RpgoPcm2D, CovarMetric) {
  RpgoConfig config;
  config.solver_type = RpgoConfig::SolverType::LEAST_SQUARES;
  config.solver_config.setLeastSquaresParamsDefault();
  config.use_pcm = true;
  config.pcm_config.metric_type = PcmConfig::MetricType::COVARIANCE;
