...
def generate_outlier_loop_closure(
    pose_graph: utils.Graph, is_3d: bool, covar: list
) -> utils.Edge:
    pose_nodes = []
    ...
    return utils.Edge(from_node, to_node, utils.EdgeType.LOOP_CLOSURE, pose, covar)


def generate_outlier_landmark_meas(
    pose_graph: utils.Graph, is_3d: bool, covar: list
) -> utils.Edge:
    inlier_edges = []
    ...
    # sample random edge to base off of
    rand_edge = random.choice(inlier_edges).copy()
    if not is_3d:
        rand_edge.pose.x *= random.uniform(1.5, 5.0)
        rand_edge.pose.y *= random.uniform(1.5, 5.0)
    else:
        rand_edge.pose.t *= np.random.uniform(low=1.5, high=5.0, size=3)
    return rand_edge


def add_outliers(pose_graph: utils.Graph, ratio: float, is_3d: bool) -> utils.Graph:
